...
export async function POST(req: NextRequest) {
  try {
    ...
    const jobLimits: Record<string, number> = {
      ...
      RECRUITER_STARTER: 3,
      RECRUITER_GROWTH: 10,
      RECRUITER_BUSINESS: 30,        // ← Updated from 99999 to 30
    ...
    if (limit === 0) {
      return NextResponse.json(
        { error: "You need a recruiter plan to post jobs.", upgradeRequired: true },
        { status: 403 }
      );
    }

    const activeJobCount = await prisma.jobPosting.count({
      where: { recruiterId: profile.id, status: "ACTIVE" },
    });
    ...
    const { title, description, requirements, location, type, salaryMin, salaryMax, deadline, status } = await req.json();

...
function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Step 3 — Invite flow: call accept API directly (no extra page/button)
      if (isInviteFlow && inviteToken) {
        try {
          ...
          if (acceptData.success) {
            // Invite accepted — go straight to recruiter dashboard
            router.push("/recruiter");
            router.refresh();
            return;
          } else {
            // Accept failed — fall back to accept page so user can retry
            console.error("Invite accept failed:", acceptData.error);
            router.push(`/recruiter/invite/accept?token=${inviteToken}`);
            return;
          }
        } catch (err) {
          // Network error — fall back to accept page

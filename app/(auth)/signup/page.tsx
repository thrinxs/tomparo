...
function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Step 2 — Sign in (needed before accept API which requires auth)
      ...
      // Step 3 — Invite flow: call accept API, then HARD redirect
      // Hard redirect (window.location.href) forces a full page reload
      // which rebuilds the JWT from scratch — at this point the
      // RecruiterTeamMember record exists so teamOwnerRole loads correctly
      if (isInviteFlow && inviteToken) {
        try {
          const acceptRes = await fetch("/api/recruiter/team/invite/accept", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          if (acceptData.success) {
            // Hard redirect — forces fresh JWT with team membership
            window.location.href = "/recruiter";
            return;
          } else {
            console.error("Invite accept failed:", acceptData.error);
            window.location.href = `/recruiter/invite/accept?token=${inviteToken}`;
            return;
          }
        } catch (err) {
    ...
    <div className="w-full max-w-md">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  ...
                  you will have access to the recruiter dashboard as a team member.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/[0.02] p-8 backdrop-blur-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isInviteFlow && (
            <p className="text-center text-xs text-slate-500">
              Your team membership will be activated automatically after signup
            </p>
          )}
        </form>

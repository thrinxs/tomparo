...
function toSafeString(value: any, field: "description" | "requirements"): string {
  ...
  if (Array.isArray(value)) {
    if (field === "requirements") {
      return value.map((r: string) => (r.startsWith("-") ? r : `- ${r}`)).join("\n");
    }
    return value.join("\n\n");
  }
  return "";
  ...
  return (
    <div className="mt-3 rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 space-y-3">
      ...
      {review.improvedContent && (
        <button
          onClick={() => onApplyFix(review.improvedContent)}
          className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-sm font-semibold hover:opacity-90 transition"
        >
...
export default function NewJobPage() {
  ...
  const handleGenerateAll = async () => {
    ...
    try {
      const res = await fetch("/api/recruiter/jobs/generate", {
        method: "POST",
  ...
  const handleGenerateField = async (field: "description" | "requirements") => {
    ...
    if (field === "description") setDescLoading(true);
    else setReqLoading(true);

    try {
      const res = await fetch("/api/recruiter/jobs/generate-field", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (field === "description") setDescription(content);
      else setRequirements(content);

      toast.success(`${field === "description" ? "Description" : "Requirements"} written!`);
    } catch (err: any) {
      toast.error(err.message || "Failed to generate");
    } finally {
      if (field === "description") setDescLoading(false);
  ...
  const handleReviewField = async (field: "description" | "requirements") => {
    ...
    if (!title.trim() || !content.trim()) {
      toast.error("Please fill in the field first");
      return;
    }
    ...
    if (field === "description") setDescReview(null);
    else setReqReview(null);

    try {
      const res = await fetch("/api/recruiter/jobs/review-field", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ field, content, title }),
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error);

      if (field === "description") setDescReview(data.review);
      else setReqReview(data.review);
  ...
  const handleSave = async (saveStatus: "ACTIVE" | "DRAFT") => {
    ...
    try {
      ...
      toast.success(saveStatus === "ACTIVE" ? "Job published!" : "Saved as draft");
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-1">
          Type a job title — then let AI write each section, or write it yourself
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 space-y-4">
        ...
        <input
          ...
          className="w-full rounded-xl border border-white/10 bg-slate-900/50 px-4 py-3 text-white placeholder-slate-500 outline-none transition focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20"
        />

        <button
          onClick={handleGenerateAll}
          disabled={generating || !title.trim()}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-semibold hover:opacity-90 transition disabled:opacity-50"
        >
          {generating ? (
            <>
        ...
        {generating && (
          <div className="flex items-center gap-3 rounded-xl border border-purple-500/20 bg-purple-500/5 px-4 py-3">
            ...
            <p className="text-sm text-purple-300">
              AI is writing your complete job posting...
            </p>
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <AIFieldButton
              ...
              onReview={() => handleReviewField("description")}
            />
          </div>
        </div>
        ...
        <textarea
          value={description}
          onChange={(e) => { setDescription(e.target.value); setDescReview(null); }}
          placeholder="Describe the role, company, and opportunity..."
          rows={8}
          className="w-full rounded-xl border border-white/10 bg-slate-900/50 px-4 py-3 text-white placeholder-slate-500 outline-none transition focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 resize-none"
        />
        ...
        {descReview && (
          <FieldReviewPanel
            review={descReview}
            onApplyFix={(improved) => {
              setDescription(toSafeString(improved, "description"));
              setDescReview(null);
              toast.success("Description updated!");
            }}
            onDismiss={() => setDescReview(null)}
          />
        )}
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <AIFieldButton
              hasContent={requirements.trim().length > 0}
              loading={reqLoading}
              onGenerate={() => handleGenerateField("requirements")}
              onReview={() => handleReviewField("requirements")}
            />
          </div>
        </div>
        ...
        <textarea
          value={requirements}
          onChange={(e) => { setRequirements(e.target.value); setReqReview(null); }}
        ...
        {reqReview && (
          <FieldReviewPanel
            review={reqReview}
            onApplyFix={(improved) => {
              setRequirements(toSafeString(improved, "requirements"));
              setReqReview(null);
              toast.success("Requirements updated!");
            }}
            onDismiss={() => setReqReview(null)}
          />
        )}
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none rounded-xl border border-white/10 bg-slate-900/50 px-4 py-3 text-white outline-none transition focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 text-sm"
              >
                {jobTypes.map((t) => (
          ...
          <div className="sm:col-span-2">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/10 bg-slate-900/50 px-4 py-3 text-white outline-none transition focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 text-sm"
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 pb-8">
        ...
        <button
          ...
          disabled={saving}
          className="flex-1 inline-flex items-center justify-center gap-2 px-6 py-4 rounded-xl border border-white/10 bg-white/5 text-white font-semibold hover:bg-white/10 transition disabled:opacity-50"
        >
          Save as Draft
        </button>
      </div>
